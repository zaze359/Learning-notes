...
> 丢帧（掉帧）不同于跳帧，丢帧并没有丢弃不再显示只是延迟显示。

丢帧就是指下一个VSYNC到来时CPU/GPU没有在一个周期内准备好新的帧数据，从而导致依然显示之前一帧的旧数据，新的一帧数据需要再等一个 vsync 才能显示，发生了延迟。

导致出现丢帧的原因：

* 在双双缓冲中由于CPU/GPU需要处理的内容过多，一帧数据无法在一个VSYNC周期内完成，导致帧率降低，感觉卡顿。
* Android 4.1之前处理 VSYNC信号时会存在由于UI绘制不及时而导致发生丢帧问题。
...
`tearing`就是指 **屏幕内显示的数据来自不同的帧**，从而画面出现撕裂感。 

导致出现画面撕裂的原因：

* **帧数据不完整**：新的帧数据并没有完全准备完毕就被用于显示。
* **在显示过程中数据发生了变化**：屏幕还在执行逐行扫描更新，但是此时缓冲区中的数据发生了变化。可能是发生缓冲区交换了，也可能是单缓冲区的情况下发生了读写并发。

若要避免出现 tearing，需要解决这两个问题，用到的是垂直同步（VSYNC）技术。
...
**VSYNC 信号是由屏幕发送的**：每次屏幕扫描完毕，并要进行下一轮扫描时会出现一个垂直同步脉冲（vertical sync pulse），通知系统屏幕开始刷新，它就是VSYNC信号。即当设备刷新频率是 60Hz时，VSYNC信号大约16.6ms发送一次。
...
> Front Buffer：用于显示；
>
> 双缓冲使得读写分离，保证数据完整性的同时也提高了性能，一定程度上降低画面撕裂的出现。
...
当GPU绘制时间过长，使得CPU + GPU的耗时超过了一个 VSYNC 信号周期时出现掉帧（帧率小于刷新率）。此时一个缓存区被GPU用于绘制，一个缓存区则被用于显示。由于 CPU 和 GPU 共用一个通道，所以将导致CPU空闲等待，且在该周期内GPU处理完成后，CPU/GPU都将空闲，基本没有做事，浪费了性能。

![img](./Android%E7%9A%84%E5%9B%BE%E5%BD%A2%E6%9E%B6%E6%9E%84.assets/551fb7b5a8a0bed7d81edde6aff99653.png)

#### 三缓冲机制（Triple Buffer）

Android 4.1 (Jelly Bean) 开始为了优化性能处理丢帧问题，启动了 Project Butter 黄油计划：包括 `VSYNC`、`Triple Buffer`和`Choreographer`。

三缓冲包含一个Front Buffer，两个Back Buffer。

在三缓冲 + VSYNC的方案中由于多了一个 Buffer 使得当新的VSYNC 到来时，即使GPU仍在处理，CPU也不会由于和GPU争抢同一个Buffer而导致空闲等待，此时CPU 可以使用新增的Buffer去立即处理数据，这样就减少了丢帧现象的出现。

* 充分发挥CPU/GPU并行处理的效率。
* 开始时掉一帧无法避免，但是可以优化后续的掉帧现象。
* 由于存在3个缓冲区，导致在**显示上存在一帧的延迟**。
...
| ViewRoot       | ViewRootImpl | 它部持有了 DecorView，并且会和WMS 、Surface等交互。          |
