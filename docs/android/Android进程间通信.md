...
* Bundle：主要是通过Intent来传递Bundle数据。常用于Activity，Service，BroadcastReceiver中。
* Messenger：和Handler、Service结合使用。它是以**串行的方式处理请求**，不适用于需要处理多线程的IPC通信场景。
* AIDL：Android接口定义语言，它是基于Binder的一套封装。适用于存在多应用访问服务，并在服务中需要多线程处理的场景。
* ContentProvider：
* 共享内存： Ashem
