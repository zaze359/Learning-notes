...
Kotlin 虽然是基于 Java6 ，但是Kotlin 中的接口方法也支持默认实现（Java8时引入的特性）。不过本质上还是 原来的接口方法，只不过使用了一种巧妙的方式来实现这个特性，可以通过转为java代码来查看具体的实现方式

* 接口参数：转为了 get 方法。所以我们定义的参数并不是一个参数，而是一个函数，因而我们不能修改值。
...
z和 密封类 具有相同的功能。它的优势在于 能帮助密封类、枚举类等类实现多继承和复杂的扩展性。
...
// 结果：
// copyIn: class java.lang.Double
// copyOut: class java.lang.Number
```

#### 协变

协变的定义是：假如类型A是类型B的子类型，那么 `Generic<A>` 也是 `Generic<B>` 的子类型。

* 通过 `out` 关键字来表明泛型类及泛型方法是协变的。对应 Java的 `<? extends X>` 。

* **只能读取**，不能添加（传出 out），泛型类型不能作为参数类型，但**可作为返回值**。（可以使用 `@UnsafeVariance` 注解取消限制）
...
假如类型A是类型B的子类型，那么`Generic<B>` 是 `Generic<A>` 的子类型。

* 使用 `in` 关键字表明泛型参数类型逆变 。对应 Java的 `<? super X>` 。

* **只能添加**，读写受限（传入 in），即泛型类型可以**作为参数类型**，但不能作为返回值。。
* **父子关系相反，支持父类型**。范围向内收拢。
...
> 协变 out 适用于泛型作为返回值的时候。对应 `getter()`
>
> 逆变 in 适用于泛型作为参数的时候。对应 `setter()`
...
* 匿名内部类：匿名内部类在初始化时会绑定父类或父接口的相应信息。（Gson使用了这种方式）

* 内联函数：编译时编译器会将相应函数的字节码插入到调用的地方。需要使用 `reified` 关键字，这样在编译时参数类型也会被插入到字节码中。
...
`{}`包裹的代码块如果访问了外部环境（即括号外）的变量），则被称为是闭包。Lambda是常见的闭包形式。
